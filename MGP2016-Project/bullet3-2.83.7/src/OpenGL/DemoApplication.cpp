...
m_cameraTargetPosition(0.f,0.7f,0.f),
m_mouseOldX(0),
m_mouseOldY(0),
m_mouseButtons(0),
m_modifierKeys(0),
...
m_frustumZNear(0.05f),
m_frustumZFar(10000.f),
m_ortho(0),
m_ShootBoxInitialSpeed(5.f),
m_stepping(true),
m_singleStep(false),
m_idle(false),
...
DemoApplication::~DemoApplication() {
	...
	if (m_shootBoxShape) delete m_shootBoxShape;
...
void DemoApplication::setCameraDistance(float dist) { m_cameraDistance  = dist; }

float DemoApplication::getCameraDistance() { return m_cameraDistance; }

void DemoApplication::toggleIdle() { m_idle = !m_idle; }
...
void DemoApplication::updateCamera() {
	...
	if (forward.length2() < SIMD_EPSILON) forward.setValue(1.f,0.f,0.f);
...
void DemoApplication::stepLeft() { m_azi -= STEPSIZE; if (m_azi < 0) m_azi += 360; updateCamera(); }
void DemoApplication::stepRight() { m_azi += STEPSIZE; if (m_azi >= 360) m_azi -= 360; updateCamera(); }
void DemoApplication::stepFront() { m_ele += STEPSIZE; if (m_ele >= 360) m_ele -= 360; updateCamera(); }
void DemoApplication::stepBack() { m_ele -= STEPSIZE; if (m_ele < 0) m_ele += 360; updateCamera(); }
...
void DemoApplication::toggleHelpText() {
	if (m_debugMode & btIDebugDraw::DBG_NoHelpText)
		m_debugMode = m_debugMode & (~btIDebugDraw::DBG_NoHelpText);
	else
		m_debugMode |= btIDebugDraw::DBG_NoHelpText;
...
void DemoApplication::setShootBoxShape() {
	if (!m_shootBoxShape) {
		btBoxShape* box = new btBoxShape(btVector3(0.1f,0.1f,0.1f));
		box->setColor(btVector3(btScalar(1.0),btScalar(0.2),btScalar(0.2)));
...
void DemoApplication::shootBox(const btVector3& destination) {
	...
	if (m_dynamicsWorld) {
		...
		btRigidBody* body = m_dynamicsWorld->localCreateRigidBody(mass,startTransform,m_shootBoxShape);
...
void DemoApplication::mouseFunc(int button, int state, int x, int y) {
	if (state == 0) m_mouseButtons |= 1<<button;
	else m_mouseButtons = 0;

	m_mouseOldX = x;
    m_mouseOldY = y;

	updateModifierKeys();
	if ((m_modifierKeys& BT_ACTIVE_ALT) && (state==0)) return;
	...
	case GLUT_LEFT_BUTTON:
		{
			if (state==0) {
				//add a point to point constraint for picking (hold left button)
				if (m_dynamicsWorld) {					
					btVector3 rayFrom;
					if (m_ortho) {
						rayFrom = rayTo;
...
void DemoApplication::mouseMotionFunc(int x,int y) {
	...
	if (m_modifierKeys & BT_ACTIVE_ALT) {
		if(m_mouseButtons & 2) {
			...
			btScalar multiplierX = btScalar(0.0002);
			btScalar multiplierY = btScalar(0.0002);
...
void DemoApplication::showProfileInfo(int& xOffset,int& yStart, int yIncr) {
	...
	if (!m_idle) time_since_reset = CProfileManager::Get_Time_Since_Reset();
...
void DemoApplication::renderscene(int pass) {
	...
	for(int i=0;i<numObjects;i++) {
		...
		if (colObj->getActivationState() == 1) { //active
			if (i & 1) wireColor += btVector3 (1.f,0.f,0.f);
			else wireColor += btVector3 (.5f,0.f,0.f);
		}
		if(colObj->getActivationState()==2) { //ISLAND_SLEEPING
			if(i&1) wireColor += btVector3 (0.f,1.f, 0.f);
			else wireColor += btVector3 (0.f,0.5f,0.f);
		}
		*/
		// Object Coloring
